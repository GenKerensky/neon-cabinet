import { Display, Input, Math as PhaserMath, Physics, Scene } from "phaser";
...
export class Game extends Scene {
  ...
  private hSpeedText!: GameObjects.Text;
  private vSpeedText!: GameObjects.Text;
  private fuelBar!: GameObjects.Graphics;
  ...
  // Background elements
  private stars!: GameObjects.Graphics;
  ...
  private createStarField(): void {
    ...
    for (let i = 0; i < 200; i++) {
      ...
      const y = PhaserMath.Between(0, height * 0.7); // More at top
      const brightness = PhaserMath.Between(30, 70);
      const color = Display.Color.GetColor(brightness, brightness, brightness);
    ...
    for (let i = 0; i < 100; i++) {
      const x = PhaserMath.Between(0, width);
      const y = PhaserMath.Between(0, height * 0.6);
      const brightness = PhaserMath.Between(80, 140);
      // Slight color variation
      const tint = PhaserMath.Between(0, 2);
    ...
    for (let i = 0; i < 40; i++) {
      const x = PhaserMath.Between(0, width);
      const y = PhaserMath.Between(0, height * 0.5);
      const brightness = PhaserMath.Between(160, 230);
      const color = Display.Color.GetColor(
        brightness,
        brightness,
        brightness + 25,
      ...
      this.stars.fillCircle(x, y, PhaserMath.FloatBetween(1, 2));
    ...
    for (let i = 0; i < 10; i++) {
      const x = PhaserMath.Between(0, width);
      const y = PhaserMath.Between(0, height * 0.4);
  ...
  private createHUD(): void {
    ...
    const dataStyle: Types.GameObjects.Text.TextStyle = {
      fontFamily: font,
      fontSize: "18px",
      fontStyle: "bold",
      color: "#00ddff",
