export const STARFIGHTER_HIGH_SCORE_KEY = "starfighterAssaultHighScore";

export function formatHighScore(score: number): string {
...
function getLocalStorageValue(): string | null {
  ...
  return localStorage.getItem(STARFIGHTER_HIGH_SCORE_KEY);
}

export function readHighScore(registry?: { get(key: string): unknown }): number {
  try {
...
export function writeHighScore(
  ...
  try {
    if (typeof localStorage !== "undefined") {
      localStorage.setItem(STARFIGHTER_HIGH_SCORE_KEY, persistedScore.toString());
    }
  } catch {
    void 0;
  }
