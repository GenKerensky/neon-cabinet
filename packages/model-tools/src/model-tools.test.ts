...
import { colorToHex, modelToObj } from "./export";
...
import { parseArgs, showHelp, showList } from "./bin/model-convert";
...
describe("model-convert", () => {
  describe("parseArgs", () => {
    ...
    it("should parse --project flag", () => {
      expect(
        parseArgs(["--project", "apps/battle-tanks", "weapons/projectile"]),
      ).toEqual({
        action: "export",
        args: ["weapons/projectile"],
        projectPath: "apps/battle-tanks",
      });
    });
  });
  ...
  describe("showHelp", () => {
    it("should not throw when called", () => {
      ...
      expect(output).toContain("Wireframe Model Converter");
    });
  });
...
describe("integration: export and import roundtrip", () => {
  ...
  it("should handle nested category paths with category-based colors", () => {
    // Create project structure with models and engine directories
    const projectRoot = join(tmpdir(), `model-test-${Date.now()}`);
    const modelsDir = join(projectRoot, "models");
    const categoryDir = join(modelsDir, "weapons");
    ...
    const { tsPath } = importModel(objPath, undefined, projectRoot);
